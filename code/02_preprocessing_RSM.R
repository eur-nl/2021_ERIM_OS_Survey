...
# identify respondents who work at RSM
RSM_respondents <-
  ERIM_OS_clean %>% 
  filter(value_1 == "Rotterdam School of Management (RSM)") %>% 
  pull(participant)

# subset RSM survey data 
RSM <- 
  ERIM_OS_clean %>% 
  filter(participant %in% RSM_respondents)

# save as .csv
write_csv(
  RSM,
  here("data", "preproc", "RSM", "RSM_ERIM_OS_Survey.csv")
)
...
write_csv(
  cluster,
  here("data", "preproc", "RSM", paste0("cluster", num_cluster, ".csv"))
)
